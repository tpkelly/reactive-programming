...
class WikipediaApiTest extends FunSuite {
  ...
  test("recovered") {
    ...
    val requests = Observable.just(1, 2, 3) ++ Observable.error(thrown)
    val responses = requests recovered
    
    val observed = Buffer[Any]()

    val sub = responses subscribe {
      observed += _
    }

    assert(observed == Seq(Success(1), Success(2), Success(3), Failure(thrown)), observed)
  }
