...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = Observable.create(o => {
      val subscribe = Reaction {
        case ValueChanged(field) => o.onNext(field.text)
        case _ => ()
      }
      
      val unsubscribe = Reaction {
        case ValueChanged => o.onCompleted
        case _ => ()
      }
      
      field.subscribe(subscribe)
      Subscription.apply(field.unsubscribe(unsubscribe))
    })
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = Observable.create(o => {
      val subscribe = Reaction {
        case ButtonClicked(button) => o.onNext(button)
        case _ => ()
      }
      
      val unsubscribe = Reaction {
        case ButtonClicked => o.onCompleted
        case _ => ()
      }
      
      button.subscribe(subscribe)
      Subscription.apply(button.unsubscribe(unsubscribe))
    })
  }
}
