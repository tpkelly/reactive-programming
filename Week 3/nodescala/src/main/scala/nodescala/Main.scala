...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val timeOut: Future[String] = Future.delay(Duration("20 seconds")).continueWith(a => "Server timeout!")

    // TO IMPLEMENT
    // 4. create a future that completes when either 20 seconds elapse
    //    or the user enters some text and presses ENTER
    val terminationRequested: Future[String] = userInterrupted.fallbackTo(timeOut)
